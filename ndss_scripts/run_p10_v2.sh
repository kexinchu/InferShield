...
PYTHON="python3"
PORT=8092
OUT="${ROOT}/ndss_scripts/results/exp10_v2"
...
start_server() {
  ...
  CUDA_VISIBLE_DEVICES="${GPUS[$model]}" \
    "${PYTHON}" -m sglang.launch_server \
      ...
      --safekv-mode "${policy}" --safekv-access-budget 100 \
      --safekv-policy-epoch 1 \
      >"${LOG}/${model}_${policy}_p10_v2_server.log" 2>&1 &
  ...
  echo "ERROR P10 server timeout: ${model}/${policy}"
  return 1
}

for model in phi4 qwen30b qwen32b; do
  for policy in strict balanced; do
    echo "P10_START ${model} ${policy}"
    if [[ -f "${OUT}/${model}_${policy}.summary.json" ]]; then
      echo "P10_SKIP_COMPLETE ${model} ${policy}"
      continue
    fi
    rm -f \
      "${OUT}/${model}_${policy}.csv" \
      "${OUT}/${model}_${policy}.summary.json"
    start_server "${model}" "${policy}"
    "${PYTHON}" "${ROOT}/ndss_scripts/exp10_attack_robustness.py" \
      --server "http://127.0.0.1:${PORT}" \
      --model "${model}" \
      --model-path "${PATHS[$model]}" \
      --policy "${policy}" \
      --n-challenges 60 \
      --output "${OUT}/${model}_${policy}.csv"
    echo "P10_POLICY_DONE ${model} ${policy}"
  done
done
