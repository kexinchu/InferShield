...
app.principal_binding = PrincipalBinding()

HEALTH_CHECK_TIMEOUT = int(os.getenv("SGLANG_HEALTH_CHECK_TIMEOUT", 20))


def _authenticate_principal(request: Request):
    """Authenticate without ever including bearer credentials in an error."""
    try:
        return app.principal_binding.authenticate(request.headers.get("Authorization"))
...
async def generate_request(obj: GenerateReqInput, request: Request):
    ...
    effective_principal = _authenticate_principal(request)
    if isinstance(effective_principal, Response):
        return effective_principal
    if effective_principal is not None:
        obj.sampling_params = app.principal_binding.bind_sampling_params(
            obj.sampling_params, effective_principal
        )
...
async def openai_v1_chat_completions(raw_request: Request):
    effective_principal = _authenticate_principal(raw_request)
    if isinstance(effective_principal, Response):
        return effective_principal
...
@app.get("/safekv/effective_principal")
async def safekv_effective_principal(raw_request: Request):
    """Expose only the authenticated principal for opt-in research validation."""
    if not app.principal_binding.enabled:
        return ORJSONResponse(content={"error": "Not found"}, status_code=404)
    effective_principal = _authenticate_principal(raw_request)
    if isinstance(effective_principal, Response):
        return effective_principal
    return {"effective_principal": effective_principal}
...
def launch_server(
    ...
    """
    app.principal_binding = PrincipalBinding.from_json_file(
        server_args.principal_binding_file
    )
    if app.principal_binding.enabled:
        logger.info(
            "Principal binding enabled for %d trusted principals",
            len(set(app.principal_binding.token_to_principal.values())),
        )
...
def _wait_and_warmup(
    ...
    # When opt-in principal binding is enabled, the local warmup request must
    # authenticate with one of the configured bearer tokens.
    elif server_args.principal_binding_file:
        with open(server_args.principal_binding_file, encoding="utf-8") as binding_file:
            binding_map = json.load(binding_file)
        warmup_token = next(iter(binding_map))
