...
class HiRadixCache(RadixCache):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            safekv_mode=getattr(server_args, "safekv_mode", "strict") if server_args else "strict",
            operator_key=getattr(server_args, "safekv_operator_key", None) if server_args else None,
            policy_epoch=getattr(server_args, "safekv_policy_epoch", 1) if server_args else 1,
            model_id=getattr(server_args, "served_model_name", None) if server_args else None,
            tokenizer_version=getattr(server_args, "tokenizer_path", None) if server_args else None,
            experiment_autoshare=getattr(server_args, "safekv_experiment_autoshare", False) if server_args else False,
        )
    ...
    def match_prefix(self, key: List[int], include_evicted=False, user_id: Optional[str] = None, **kwargs):
        ...
        # Namespace-aware lookup (private / public / budgeted). The previous
        # global-root walk ignored SafeKV isolation.
        value, last_node = RadixCache.match_prefix(
            self, key, user_id=user_id, **kwargs
        )

        last_node_global = last_node
        while getattr(last_node, "evicted", False) and last_node.parent is not None:
            last_node = last_node.parent
    ...
    def _match_prefix_helper(self, node: TreeNode, key: List, user_id: Optional[str] = None):
        ...
        return RadixCache._match_prefix_helper(self, node, key, user_id)
