...
class ServerArgs:
    ...
    principal_binding_file: Optional[str] = None
    ...
    safekv_mode: Literal["strict", "balanced", "legacy"] = "strict"
    safekv_operator_key: Optional[str] = None
    safekv_policy_epoch: int = 1
    # Experiment-only: admit new Balanced nodes as Budgeted-Shared without
    # waiting for the detector. Used to measure the B/ledger reuse axis.
    safekv_experiment_autoshare: bool = False
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--safekv-access-budget",
            type=int,
            default=ServerArgs.safekv_access_budget,
            help="Balanced-mode cumulative cross-tenant access budget.",
        )
        parser.add_argument(
            "--safekv-creator-threshold",
            type=int,
            default=ServerArgs.safekv_creator_threshold,
            help="Legacy creator threshold; never authorizes Verified-Public.",
        )
        parser.add_argument(
            "--safekv-mode",
            choices=["strict", "balanced", "legacy", "none"],
            default=ServerArgs.safekv_mode,
            help=(
                "SafeKV visibility policy. Security experiments use strict. "
                "Use 'none' for a vanilla SGLang baseline (global cache sharing, no namespace isolation)."
            ),
        )
        parser.add_argument(
            "--safekv-operator-key",
            type=str,
            default=ServerArgs.safekv_operator_key,
            help="Operator HMAC key for Public authorizations.",
        )
        parser.add_argument(
            "--safekv-policy-epoch",
            type=int,
            default=ServerArgs.safekv_policy_epoch,
            help="Current Public Registry policy epoch.",
        )
        parser.add_argument(
            "--safekv-experiment-autoshare",
            action="store_true",
            help=(
                "Experiment only: in Balanced mode, insert nodes as "
                "Budgeted-Shared so the B/ledger reuse axis can be measured "
                "without waiting for the conservative detector."
            ),
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default=ServerArgs.principal_binding_file,
            help=(
                "Opt in to trusted principal binding with a JSON object mapping "
                "opaque bearer tokens to principal IDs. Applies to /generate and "
                "/v1/chat/completions."
            ),
        )
        parser.add_argument(
...
class PortArgs:
    ...
    private_judge_to_server: str
    private_judge_to_client: str
    ...
    def init_new(server_args, dp_rank: Optional[int] = None) -> "PortArgs":
        ...
        if not server_args.enable_dp_attention:
            ...
            return PortArgs(
                ...
                private_judge_to_server=f"ipc://{tempfile.NamedTemporaryFile(delete=False).name}",
                private_judge_to_client=f"ipc://{tempfile.NamedTemporaryFile(delete=False).name}",
