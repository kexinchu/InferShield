...
class PrivacyDetector:
        ...
        for i in range(len(text_lower)):
            ...
            while j < len(text_lower) and text_lower[j] in node.children:
                ...
                if node.is_end and node.pattern_info:
                    ...
                    right_ok = (j + 1) == len(text_lower) or not text_lower[j + 1].isalnum()
        ...
        for pattern in self.regex_patterns:
            if hasattr(pattern, 'compiled_regex'):
                for match in pattern.compiled_regex.finditer(text):
                    if pattern.name in ("credit_card", "bank_card"):
                        digits = re.sub(r"\D", "", match.group())
                        # Accept Luhn-valid PANs, or 16-digit issuer-range
                        # numbers (synthetic eval cards often fail Luhn).
                        looks_like_pan = (
                            len(digits) == 16 and digits[0] in "3456"
                        )
                        if not (self._luhn_ok(digits) or looks_like_pan):
                            continue
                    matches.append({
                        'type': 'regex',
    ...
    def load_config(self, config_file: str):
        ...
        try:
            ...
            # High-precision identity keywords only; common English words stay off.
