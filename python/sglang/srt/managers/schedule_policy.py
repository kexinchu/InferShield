...
class SchedulePolicy:
    ...
    def __init__(
        ...
        # In-batch LPM tree must use the same isolation mode as the
        # serving cache. Defaulting to "strict" while the main tree is
        # "none" (or the reverse) lets a concurrent probe inherit another
        # principal's prefix length.
        self.waiting_queue_radix_tree = RadixCache(
            ...
            safekv_mode=getattr(tree_cache, "safekv_mode", "strict"),
        )
            ...
            if self.enable_hierarchical_cache:
                r.prefix_indices, r.last_node, r.last_node_global = (
                    self.tree_cache.match_prefix(
                        key=prefix_ids,
                        user_id=r.user_id,
                        authorization=r.safekv_public_authorization,
            ...
            if len(r.prefix_indices) <= IN_BATCH_PREFIX_CACHING_CHECK_THRESHOLD:
                in_batch_matching_prefixes, _ = (
                    self.waiting_queue_radix_tree.match_prefix(
                        rid=r.rid, key=prefix_ids, user_id=r.user_id
                    )
                )
                if (
